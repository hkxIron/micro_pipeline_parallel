...
Both are required. Here's why each fails without the other:

---

## Scenario: Rank 0 and Rank 1 in Steady State (1F1B)
...
**Why you don't need `req.wait()`:**
- The `async_requests` list keeps all request objects alive
- Function scope keeps the list alive until return
- By the time the function returns, all async operations have completed
- (The receiving side's `recv()` calls ensure sends complete)

---

## Visual Timeline: Why Both Are Needed

### Blocking Forward (DEADLOCK):
```
Rank 0: [Fwd] → [send_forward BLOCKS] → [waiting...] → [can't recv_backward]
Rank 1: [waiting for recv_forward] → [can't send_backward] → [DEADLOCK]
```

### Async Without Saving (CRASH):
```
Rank 0: [Fwd] → [isend starts] → [req GC'd] → [Gloo accesses freed buffer] → CRASH
```

### Async With Saving (WORKS):
```
Rank 0: [Fwd] → [isend starts] → [req saved] → [recv_backward] → [continues]
Rank 1: [recv_forward] → [Fwd] → [send_backward] → [continues]
Both make progress! ✅
```
